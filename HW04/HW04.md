...
- Change the IP address of a connection (temporarily) using nmcli
    - Dùng lệnh:
        ...
        ```
    - Giải thích: Lệnh `nmcli connection modify` thay đổi cấu hình IP thành địa chỉ tĩnh, sau đó `nmcli connection up` kích hoạt lại kết nối để áp dụng thay đổi.
    - Kết quả:
...
- Open nmtui
    - Dùng lệnh:
        ...
        sudo nmtui
        ```
    - Giải thích: Lệnh `nmtui` mở giao diện text-based interactive để cấu hình mạng dễ dàng hơn.
    - Kết quả:
